...
class AFP:
    def __init__(self, data_path=None, start_date='2000-01-03', end_date='2002-12-31',
                 initial_price=1, rolling='max'):
    ...
    def calculate_mean_variance_returns(self, method='max_quadratic_utility', risk_aversion=1,
                                        market_neutral=True):
        ...
        for ix in range(start_index, end_index+1):
            if rolling != 'max':
                ...
                start_ix = 0
            data = rets.loc[rets.index[start_ix:ix]]
            ...
            ret = rets.loc[rets.index[ix]].values.dot(weights_df)
    ...
    def calculate_equal_weight_returns(self):
        ...
        for ix in range(start_index, end_index+1):
            ret = rets.loc[rets.index[ix]].values.dot(weights)
    ...
    def compare(self, weights):
        ...
        merged_rets_df = pd.merge(merged_df, drl_rets_df, left_index=True, right_index=True)
        self.merged_rets_df = merged_rets_df
        self.cum_rets_df = ((1+merged_rets_df).cumprod()-1)
